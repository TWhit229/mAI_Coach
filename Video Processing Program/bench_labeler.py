...
class LabelerApp:
    def __init__(self, root, video_paths: List[Path], dataset_dir: Path):
        ...
        self.current_frame = 0      # frame we're on
        self.total_frames = 0
        self.playing = False        # start paused; button shows "Play"
        self.tk_img = None
        self.fps = 30.0

        self._updating_scale = False  # guard to avoid scrub callback loops
    ...
    def load_video(self, index: int):
        ...
        # total frames for scrubber
        if self.dataset and "frames" in self.dataset:
            self.total_frames = len(self.dataset["frames"])
        else:
    ...
    def on_scrub(self, value):
        """User moved the time slider; jump to that frame."""
        if self.cap is None:
            return
        try:
            idx = int(float(value))
        ...
        # No auto-pause here; if it's playing, it will just jump to this frame
    ...
    def play_loop(self):
        ...
        if self.cap is not None and self.dataset is not None and self.total_frames > 0:
            # seek to current_frame and show it
            fi = max(0, min(self.current_frame, self.total_frames - 1))
            ...
            if ok:
                ...
                if 0 <= fi < len(frames):
                    ...
                    if frec.get("pose_present") and frec.get("landmarks"):
                        draw_upper_body_overlay(frame_disp, frec["landmarks"])
                ...
                if self.playing:
                    self.current_frame = fi + 1
                    if self.current_frame >= self.total_frames:
                        # stop on last frame; or loop if you prefer
    ...
    def save_and_next(self):
        if self.cap is None or self.dataset is None:
            return
